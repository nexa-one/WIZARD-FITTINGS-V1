...
export const EnterDimensions: React.FC<Props> = ({ fittingType, values, onChange }) => {
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="text-xs font-semibold text-gray-400">{t.inletDimensions}</div>
      {!isRound ? (
        <div className="grid grid-cols-2 gap-2">
          <div>
            <label className={label}>{t.dimensionWidth}</label>
            <input type="number" className={input} value={values.inletWidth} onChange={(e) => set('inletWidth', Number(e.target.value))} />
          </div>
          <div>
            <label className={label}>{t.dimensionHeight}</label>
      ...
      {hasOutlet && (
        <>
          <div className="text-xs font-semibold text-gray-400">{t.outletDimensions}</div>
          {!isRound ? (
            <div className="grid grid-cols-2 gap-2">
              <div>
                <label className={label}>{t.dimensionWidth}</label>
            ...
            <div>
              <label className={label}>{t.outletDiameterLabel}</label>
