...
export const OrderPanel: React.FC = () => {
  ...
  return (
    <DockablePanel title="Order Panel" defaultDocked="right" className="w-64">
      <div className="p-3 space-y-3">
        <div className="bg-gray-800 rounded p-2">
          <div className="text-xs text-gray-400 mb-1">Current Order</div>
          {currentOrder ? (
            <>
            ...
            <div className="text-xs text-gray-500">No active order</div>
          )}
        </div>
        ...
        {currentOrder && currentOrder.items.length > 0 && (
          <div className="space-y-1">
            <div className="text-xs font-semibold text-gray-400 uppercase">Items</div>
            {currentOrder.items.map((item, i) => (
              <div key={i} className="bg-gray-800 rounded p-2 text-xs">
                ...
                <div className="text-gray-500">Gauge: {item.psd.wallGauge}</div>
              </div>
            ))}
          </div>
        )}
        ...
        {orders.length > 0 && (
          <div className="space-y-1">
            <div className="text-xs font-semibold text-gray-400 uppercase">All Orders</div>
            {orders.map((order) => (
