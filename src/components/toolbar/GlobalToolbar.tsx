  ...
  return (
    <div className="flex items-center gap-3 px-4 py-2 bg-gray-900 border-b border-gray-700 min-h-[52px] flex-wrap">
      <div className="flex items-center gap-2 mr-4">
        ...
        <span className="text-white font-bold text-sm whitespace-nowrap">{t.appTitle}</span>
      </div>
      ...
      <div className="flex items-center bg-gray-800 rounded-full p-0.5 gap-0.5">
        {(['3D_VISUAL', 'CNC_FLAT_PATTERN', 'X_RAY_TRANSPARENT'] as const).map((mode) => (
          ...
          >
            {mode === '3D_VISUAL' ? t.view3D : mode === 'CNC_FLAT_PATTERN' ? t.viewCNC : t.viewXRay}
          </button>
        ))}
      </div>
      ...
      <div className="flex items-center gap-2">
        <label className="text-xs text-gray-400 whitespace-nowrap">{t.gaugeLabel}</label>
        <input
          type="number"
          value={waterGauge}
      ...
      <div className="ml-auto flex items-center gap-2">
        {currentOrder && (
          <span className="text-xs text-gray-400">
            {t.orderLabel} <span className="text-blue-400 font-semibold">{currentOrder.orderId}</span>
            <span className="text-gray-500 ml-1">{t.itemsCount(currentOrder.items.length)}</span>
          </span>
        )}
      </div>
    </div>
