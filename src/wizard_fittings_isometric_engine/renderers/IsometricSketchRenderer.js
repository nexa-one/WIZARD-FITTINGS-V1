...
export class IsometricSketchRenderer {
  ...
  _drawFaces(faces, pts, theme) {
    const ctx = this.ctx;
    // Fix 2: 'front' face (duct inlet, −Y normal) appears on the viewer's LEFT
    // in the 30° isometric projection, so it must use theme.face_left (lighter
    // shade), not theme.face_right (darker shade).
    const shadeMap = {
      top:    theme.face_top,
