...
class TestIsometricProjection:
    ...
    def test_zero_scale_raises(self):
        with pytest.raises(ValueError):
            IsometricProjection(scale=0.0)

    def test_negative_scale_raises(self):
        with pytest.raises(ValueError):
            IsometricProjection(scale=-50.0)
...
class TestFittingExporter:
    ...
    def test_non_dict_sketch_raises(self):
        with pytest.raises(ExportError):
            self.exp.export("not-a-dict", fmt="pdf")  # type: ignore[arg-type]

    def test_none_sketch_raises(self):
        with pytest.raises(ExportError):
            self.exp.export(None, fmt="pdf")  # type: ignore[arg-type]