...
class Order(UUIDMixin, TimestampMixin, Base):
    ...
    job_id: Mapped[uuid.UUID | None] = mapped_column(
        UUIDType(), ForeignKey("jobs.id"), nullable=True
    )
    job_area_id: Mapped[uuid.UUID | None] = mapped_column(
        UUIDType(), ForeignKey("job_areas.id"), nullable=True
    )
    ...
    urgency: Mapped[UrgencyLevel] = mapped_column(PortableEnum(UrgencyLevel), default=UrgencyLevel.NORMAL)
    piece_quantity: Mapped[int] = mapped_column(Integer, default=1, nullable=False)
    status: Mapped[OrderStatus] = mapped_column(PortableEnum(OrderStatus), default=OrderStatus.DRAFT, nullable=False, index=True)
    order_type: Mapped[OrderType] = mapped_column(PortableEnum(OrderType), default=OrderType.STANDARD, nullable=False)
    notes: Mapped[str | None] = mapped_column(Text)
    tags: Mapped[list] = mapped_column(JSONBType(), default=list, nullable=False)
    audit_history: Mapped[list] = mapped_column(JSONBType(), default=list, nullable=False)
...
class OrderItem(UUIDMixin, TimestampMixin, Base):
    ...
    fitting_id: Mapped[uuid.UUID | None] = mapped_column(
        UUIDType(), ForeignKey("fittings.id"), nullable=True
    )
    ...
    specifications: Mapped[dict] = mapped_column(JSONBType(), default=dict, nullable=False)
